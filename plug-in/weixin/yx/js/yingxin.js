...
function isTime() {
	var beginDate = new Date("2017-05-20");
	var endDate = new Date("2017-06-20");
	var date = new Date();
	...
	$.ajax({
			...
			success: function(data) {
				...
				$("#gerenxinxibut").on("click", function() {
						...
						console.log(JSON.stringify(grinfo));
						console.log(JSON.stringify(jtinfo))
						var otjt1;
						...
						if(mobile == "" || stmail == "" || syd == "" || jtdz == "" || xssg == "" || xstz == "" || fzcc == "none" || xzcc == "none") {
							alert('带"*"的为必填项');
						} else {
								...
								if(sfjf == "Y") {
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(data) {
                                            ...
                                            if(bmsu == true) {
                                                //判断是否单招
                                                if(sfdz="Y"){//是单招生，则要判断是否在时间内
                                                    //判断单招生是否在范围内 开始
                                                    isTime();
                                                    if(isTimeIn==true){
                                                        //报名成功并且在范围内生成班级开始 按照正常流程，可以进入下一页
                                                        $.ajax({
                                                            type: "post",
                                                            dataType: "json",
                                                                        ...
                                                                        success: function(data) {
                                                                            ...
                                                                            if(obj.length != 0) { //判断是否有床位
                                                                                if(end != null && end == "end") { //判断是否为END
                                                                                    ...
                                                                                    var drr = $("#qinshi00 option:selected").val();
                                                        ...
                                                        //生成班级结束
                                                    }else{//单招生不在范围内，不能选宿舍，不能进入下一页
                                                      //  alert("请在时间范围内("+kssj+"至"+jssj+")进行下一步操作！");
                                                        alert("请在时间范围内进行下一步操作！");
                                                    }//判断单招生是否在范围内 结束
...
//绿色通道 缴费下拉箭头
$(".feiyongxinxi li").children(".feiyong-list-wrap").children(".feiyong-con").children(".xiala").addClass("xiala-down"); $(".feiyongxinxi li").click(function() {
...
var val = $('input:radio[name="chuang"]:checked').val(); $(".shi-chuang").children(".radio-but").addClass("radio-but-on"); $(".shi-chuang").click(function() {
...
//个人信息页面家庭成员
$(".jiating-list-wrap").children(".jiating-con").children(".xiala").addClass("xiala-down"); $(".jiating-list-wrap").click(function() {
